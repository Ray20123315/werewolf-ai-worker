...
test("Masochist addon keeps one normal ordinary ballot per the latest Word-source main", () => {
  ...
  assert.equal(snapshot.entries.find((entry) => entry.voterId === "m")?.status, "valid");
  assert.equal(snapshot.counts.w, 2);
